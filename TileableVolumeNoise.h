...
static float TileableWorleyNoise3D(float px, float py, float pz, float cellCount);

/// @return Tileable Perlin noise value in [0, 1].
/// @param p 3d coordinate in [0, 1], being the range of the repeatable pattern.
static float TileablePerlinNoise3D(float px, float py, float pz, float frequency, int octaveCount);
